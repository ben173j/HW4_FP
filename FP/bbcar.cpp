#include "bbcar.h"
extern parallax_encoder ENCODER;
 
//BBCar::BBCar( PwmOut &pin_servo0, PwmOut &pin_servo1, Ticker &servo_ticker ):
BBCar::BBCar( PwmOut &pin_servo0, PwmOut &pin_servo1, Ticker &servo_ticker, DigitalIn &encoder_in, Ticker &encoder_ticker ):

servo0(pin_servo0), servo1(pin_servo1), ENCODER(encoder_in,encoder_ticker)
{
    servo0.set_speed(0);
    servo1.set_speed(0);
...
void BBCar::goStraight( double speed ){\
   ...
   // ENCODER.count_steps();
    //printf("(BBCAR) distance: %0.95f\n\r",ENCODER.get_cm());
    //printf("(BBCAR) steps: %d\n\r",ENCODER.get_steps());
}
...
void BBCar::reverseturn(double speed, double factor){
    if(factor>0){
        servo0.set_factor(factor*0.95);
        servo1.set_factor(1);
    }
