...
class BBCar{
	public:
		BBCar( PwmOut &pin_servo0, PwmOut &pin_servo1, Ticker &servo_ticker,DigitalIn &encoder_in, Ticker &encoder_ticker );
		//BBCar( PwmOut &pin_servo0, PwmOut &pin_servo1, Ticker &servo_ticker);

		parallax_servo servo0;
		parallax_servo servo1;
		parallax_encoder ENCODER;
		...
		// for reverse turn left/right with a factor of speed
		void reverseturn(double speed, double factor);
